...
# #             'is_predefined': False,
# #             'is_active': True,
# #             'email_verified': True,  # Directly set to True without OTP
# #             'last_login': None,
# #             'created_at': datetime.utcnow(),
# #             'updated_at': datetime.utcnow()
# #         }
...
# #             'user_id': user_id,
# #             'email': data['email'],
# #             'requires_verification': False  # Always false now
# #         }
...
#                 f'New appointment request from {data.get("patient_name")} for {data.get("therapy_type")} therapy. Please review and confirm.',
#                 'info'
#             )
...
#                     doctor['user_id'],
#                     'New Appointment Request',
#                     f'New appointment request from {data.get("patient_name")} for {data.get("therapy_type")}. Waiting for confirmation.',
#                     'info'
#                 )
...
#         # Get appointment details for notifications and receipt
...
#                     f'Your {appointment.get("therapy_type", "therapy")} appointment has been confirmed by receptionist. Date: {appointment.get("date").strftime("%d %b %Y at %I:%M %p")}. You can download your receipt now.',
#                     'success'
#                 )
...
def patient_dashboard():
    ...
    if session.get('role') != 'patient':
        return jsonify({'error': 'Access denied. Patient access only.'}), 403
...
def doctor_dashboard():
    ...
    if session.get('role') != 'doctor':
        return jsonify({'error': 'Access denied. Doctor access only.'}), 403
...
def receptionist_dashboard():
    ...
    if session.get('role') != 'receptionist':
        return jsonify({'error': 'Access denied. Receptionist access only.'}), 403
